...
ns.AddEncounter(2070,{
	...
	Settings = {
		['tankCircle'] = { 
			['circle'] = { enable = true, name = "Военный совет Анторуса", customName = 'Текущий танк', desc = 244892 }, 
		}, 
		[244737] = {
			['circle'] = { enable = true, color = 3, desc = 244737, },
		},
	},
		...
		if event == "COMBAT_LOG_EVENT_UNFILTERED" then 
			...
			if eventType == "SPELL_AURA_APPLIED" then
				if spellID == 244737 then
					if OC(244737) then
						ns.AddSpinner(dstGUID, 244737, 3, { GetTime(), 5 }, 120 )
					end
				end
			elseif eventType == "SPELL_AURA_REMOVED" then
				if spellID == 244737 then
					ns.RemoveSpinner(dstGUID, 244737)
				end
			end
		end
	end,
